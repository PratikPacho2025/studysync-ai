...
export function RevisionPage() {
  ...
  function loadRevisions() {
    api.fetchRevisions().then(setRevisions).catch(console.error)
  }
  ...
  // Calculate next date helper locally for the UI sheet
  const selectedNextDate = useMemo(() => {
    if (!selectedRevision) return ''
    const revisionIntervals = [6, 24, 72, 168, 360]
    const hours = revisionIntervals[Math.min(selectedRevision.revisionNumber - 1, revisionIntervals.length - 1)] ?? 360
    const d = new Date(selectedRevision.scheduledDate)
    d.setHours(d.getHours() + hours)
    return formatRevisionDate(d.toISOString(), { weekday: 'short', year: 'numeric' })
  }, [selectedRevision])
  ...
  function saveCompletion(reflection) { 
    const revision = selectedRevision
    api.completeRevision(revision.id, reflection).then((res) => {
      loadRevisions()
      setLastCompleted({ revision, nextDate: res.nextDate })
